	...
	<div class="mb-10 flex flex-col gap-1.5 text-lg">
		<div class="flex gap-3">
			<div class="flex items-center gap-1">
				...
				<span class="font-medium">{globalTranslations[lang].duration}</span>
			</div>
			<span class="text-gray-700">{duration} {globalTranslations[lang].hours}</span>
		</div>
		<div class="flex gap-3">
			<div class="flex items-center gap-1">
				<GaugeIcon class="h-5 w-7" />
				<span class="font-medium">{globalTranslations[lang].level}</span>
			</div>
	...
	<div class="mx-auto mt-6 mb-16 w-full max-w-6xl gap-20 px-6 md:mt-12">
		...
		<div class="flex flex-col-reverse md:grid md:grid-cols-3 md:gap-10">
			<div class="col-span-2 max-w-156 text-lg">
				...
				<div class="mb-10">
					{@render label(globalTranslations[lang].summary)}
					<p class="text-gray-700">{translation?.summary}</p>
				</div>
				<div class="mb-10">
					{@render label(globalTranslations[lang].objectives)}
					<p class="text-gray-700">{translation?.objectives}</p>
				</div>
				<div class="mb-10">
					{@render label(globalTranslations[lang].topics)}
					<ul class="space-y-1">
				...
				<div class="mb-10">
					{@render label(globalTranslations[lang].methods)}
					<p class="text-gray-700">{translation?.methods}</p>
				</div>
				<div class="mb-10">
					{@render label(globalTranslations[lang].prerequisites)}
					<p class="text-gray-700">{translation?.prerequisites}</p>
				</div>
				<div>
					{@render label(globalTranslations[lang].references)}
					<ul class="space-y-1">
			...
			<div class="mb-12">
				<h3 class="mb-4 text-lg font-medium">{globalTranslations[lang].instructors}</h3>
